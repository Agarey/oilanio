  ...
  return (
    <div className={classnames(styles.container, props.course.approved ? null : styles.moderate, props.course.is_archived === true ? styles.moderate : null)}>
      <ModalWindow 
        ...
        body={<CardCreationPermissionForm permittedCardsCount={permittedCardsCount} handleClose={handleClose}/>}
      />
      ...
      <div className={styles.info_block}>
        <div>
          <span style={{color: '#FF8300', fontWeight: 'bold' }}>
            {props.course.approved ? null : 'Карта находится в модерации'}
          </span>
          <span style={{color: '#FF8300', fontWeight: 'bold' }}>
            {props.course.is_archived === true ? 'Карта находится в архиве' : null }
          </span>
          ...
          >
            {edit 
              ? (props.courseCategories === undefined 
              ? null 
              : (props.courseCategories.map(category => category.name === "test" 
                ? null 
                : <option value={category.id}>
                  {category.name}
                </option>)))
                : <option value={props.course.category_id}>
                  {props.course.category_name}
                </option>}
          </select>
          <div>
            <p className={styles.info_small}>
              {props.course.is_online}
            </p>
          ...
          <p className={styles.info_price}>
            Цена: {price} {currency} / {unitOfTime}
          </p>
          ...
          <button className={styles.info_button}
            ...
            >
              {edit ? 'Сохранить изменения' : 'Редактировать'}
            </button>
          </div>
          <div className={styles.btnsWrapper}>
