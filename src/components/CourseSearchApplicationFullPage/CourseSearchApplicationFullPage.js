...
export default function CourseSearchApplicationFullPage(props) {
  ...
  const [firstStepValidationState, setFirstStepValidationState] =
    useState(false);
  ...
  const loadData = async () => {
    ...
    let imagesBaseLocalStorage = JSON.parse(
      localStorage.getItem(globals.localStorageKeys.imagesBase)
    );
    if (imagesBaseLocalStorage == null) {
      let imagesBaseResponse = await axios.get(
        `${globals.productionServerDomain}/imagesBase`
      );
  ...
  const getCards = async () => {
    ...
    let postResult = await axios.post(
      `${globals.productionServerDomain}/${
        props.searchCenter ? "courseCardsFilter" : "tutorCourseCardsFilter"
      }/`,
      data
    );
        ...
        <div className={styles.header}>
          ...
          <h1 className={styles.main_title}>
            {step === 1 ? (
              ...
              <>
                <span className={styles.orange}>Шаг 2 –</span> Введите ваши
                персональные данные
              </>
            )}
          </h1>
        ...
        <ModalWindow
          ...
          body={
            <div style={{ color: "#000", fontSize: 16, textAlign: "center" }}>
              <p>
                Сообщение со входом в личный кабинет <br /> было отправлено на
                вашу почту. <br />В ближайшее время с вами свяжутся
                центры/репетиторы.
              </p>
              <button className={styles.downButton} onClick={handleClose}>
                Понятно
              </button>
            </div>
          }
  ...
  function secondStep() {
    return (
      <>
        ...
        >
          {submitButtonPressed ? "Подобрать курсы!" : "Подобрать курсы"}
        </button>
        <span className={styles.message}>
          {messageForUser}
          <br />
    ...
    return (
      <div className={styles.whiteBody}>
        ...
        <div className={styles.header}>
          ...
          >
            Мы подобрали вам{" "}
            {props.searchCenter ? "следующие курсы" : "следующих репетиторов"}!
          </h1>
        </div>
        {loading
          ? null
          : courseCards.length === 0 && (
              <CourseSearchResultIsNotDefind setStep={setStep} />
            )}
            ...
            courseCards.splice(0, courseCards.length).map((item) => {
              if (props.searchCenter) {
                if (item.title != "test") {
                  return (
                    <CourseCard
                      coverImage={
                        imagesBase[
                          Math.floor(Math.random() * imagesBase.length)
                        ].src
                      }
                ...
                return (
                  <TutorCourseCard
                    coverImage={
                      imagesBase[Math.floor(Math.random() * imagesBase.length)]
                        .src
                    }
                    showApplicationModal={true}
                    sendApplicationCallback={sendApplication}
  ...
  return step === 1 || step === 2 ? Main() : lastStep();
}
