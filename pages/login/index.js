...
function SignIn(props) {
  ...
  async function register(
    ...
    const result = await axios.post(
      globals.productionServerDomain + "/courses/register",
      data
    );
  ...
  const apper = () => {
    ...
    if (interfaceForStudent) {
      if (studentPhone.length !== 16) {
        ...
        axios
          .post(`${globals.productionServerDomain}/studentLogin/`, {
            phone: studentPhone,
          })
          .then(function (res) {
      ...
      axios
        .post(`${globals.productionServerDomain}/login/`, data)
        .then(function (res) {
          if (res.status === 200) {
            let date = new Date();
            let loginObject = {
    ...
    <div>
      ...
      >
        ...
        <span style={{ fontSize: "12px" }}>
          9:00-17:00 праздничные и выходные дни
        </span>{" "}
        <br /> <br />
        <span style={{ fontSize: "14px", fontWeight: "bold" }}>
          Телефоны:
        ...
        <br />
        <span style={{ fontSize: "12px" }}>
          +7 (747) 095 3440 (WhatsApp, Звонок)
        </span>{" "}
        <br />
      </div>
      <div className={styles.signInBody}>
        ...
        <div className={classnames(styles.formBody)}>
          <div className={styles.header}>
            <div
              className={classnames(
                styles.headerItem,
                styles.headerItemLeft,
                !interfaceForStudent && styles.selected
              )}
              onClick={() => {
          ...
          <div className={styles.form} onKeyPress={handleAnswerChange}>
              ...
              <>
                <div className={styles.inputBody}>
                  <input
                    ...
                    onChange={(e) =>
                      globals.checkPhoneMask(e.target.value, setStudentPhone)
                    }
            ...
            <div className={styles.inputBody} style={{ marginTop: "15px" }}>
              <button className={styles.submitBtn} onClick={apper}>
