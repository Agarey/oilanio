...
export default function Home() {
    ...
    const [currentCourseCategory, setCurrentCourseCategory] = useState("Популярные курсы")
    ...
    const [courseSearchMessageForUser, setCourseSearchMessageForUser] = useState(null);
    const [courseSearchSubMessageForUser, setCourseSearchSubMessageForUser] = useState(null);
    const [courseSearchSubmitButtonPressed, setCourseSearchSubmitButtonPressed] = useState(false);
    ...
    const loadCourseCards = async (directionId) => {
        if(directionId !== undefined){
            let result = await axios.get(`${globals.productionServerDomain}/courseCards/` + directionId);
    ...
    useEffect(async () => {
        ...
        let promotionsResult = await axios.post(`${globals.productionServerDomain}/loadDirectionPromotions`, {direction_id: 0});
    ...
    return (
        <div
             style={{background: 'linear-gradient(90deg, rgba(60,88,185,1) 0%, rgba(119,148,248,1) 100%)'}}
        >
            ...
            <div className={coursesStyles.topHeader} id={'filter'}>
                <div className={coursesStyles.topHeaderBody}>
                    ...
                    <img src="/Cone-Blue-Glossy.png" className={coursesStyles.ConeBlue} alt=""/>
                    ...
                    <div className={coursesStyles.filter}>
                        <div className={coursesStyles.filterBlock}>
                            <select name="citySelect" id="citySelect" className={coursesStyles.selectBlock}>
                            ...
                            <select name="isOnlineSelect" id="isOnlineSelect" className={coursesStyles.selectBlock}>
            ...
            <div className={coursesStyles.sortBlock} style={{height: openMoreSort ? 'auto' : 0, padding: openMoreSort ? '20px' : 0}}>
                <div className={coursesStyles.sortItem}>
                    <span className={classnames(coursesStyles.sortItemTitle)}>Сортировать по:</span>
                    <span className={classnames(coursesStyles.sortValue, sortType===1 ? coursesStyles.active : null)} onClick={()=>{
                        setSortType(1)
                    }}>Цене (по возрастанию)</span>
                    <span className={classnames(coursesStyles.sortValue, sortType===2 ? coursesStyles.active : null)} onClick={()=>{
                        setSortType(2)
                    }}>Цене (по убыванию)</span>
                    <span className={classnames(coursesStyles.sortValue, sortType===3 ? coursesStyles.active : null)} onClick={()=>{
                        setSortType(3)
                    }}>Рейтингу</span>
                    <span className={classnames(coursesStyles.sortValue, sortType===4 ? coursesStyles.active : null)} onClick={()=>{
                        setSortType(4)
                    }}>Популярности</span>
                </div>
                <div className={coursesStyles.sortItem}>
                    <span className={coursesStyles.sortItemTitle}>Вид занятий:</span>
                    <span className={classnames(coursesStyles.sortValue, lessonType===1 ? coursesStyles.active : null)} onClick={()=>{
                        setLessonType(1)
                    }}>Индивидуальные занятия</span>
                    <span className={classnames(coursesStyles.sortValue, lessonType===2 ? coursesStyles.active : null)} onClick={()=>{
                        setLessonType(2)
                    }}>Групповые занятия</span>
                </div>
            ...
            <div id={'coursesBlock'}
                 className={classnames(coursesStyles.container)}>
                {loading ? (
                    ...
                    (
                        <>
                            <div className={coursesStyles.img_title} style={{margin: '0 0'}} id={'akcii'}>
                                            ...
                                            if(course.title !== 'test'){
                                                return (
                                                    <div style={{marginLeft: '5%', marginRight: '5%'}}>
                                                        <CourseCard setLoadingModal={setLoadingModal} course={course}/>
                                                    </div>
                                                )
                                            }
                                        })
                                        :
                                        (<p className={styles.message}>По данным фильтрам пока что нет курсов! :(</p>)
                                }
                            </div>
                        </>
                    )
                ...
                {filters[1] !== undefined ?
                    <div style={{width: '100%'}}>
                        <div className={styles.courses}>
                            {
                                filters[1] !== undefined
                                    ?
                                    (filters[1].slice(categoriesFirstValue, categoriesSecondValue).map(filterOption => ( filterOption.id !== 0 ? (
                                        <Link href={`/?direction=${filterOption.id}&city=0&price=0&isOnline=0`}>
                        ...
                        <div className={styles.centerTitleBlock} style={{marginTop: 20}}>
                            ...
                            <button className={classnames(styles.arrow)} disabled={categoriesSecondValue >= filters[1].length ? true : false} onClick={()=>{
                    ...
                    : null}
                <br/>
                <CourseSearchFormBlock yandexMetricaFunction={ym} directions={filters[1]}/>
            </div>
            ...
            <div className={coursesStyles.whyUs}>
                <div className={coursesStyles.whyUsHeader}>
                    <span className={coursesStyles.whyUsTitle}>Почему лучше выбрать курс с Oilan</span>
                </div>

                <div className={coursesStyles.container} style={{paddingBottom: 0}} style={{background: 'transparent'}}>
                    <div className={coursesStyles.whyUsContentBlock}>
                        <div className={coursesStyles.whyUsCard}>
                            ...
                            <div className={coursesStyles.whyUsCardDescBody}>
                                <span className={coursesStyles.whyUsCardTitle}>Система подбора</span>
                                <span className={coursesStyles.whyUsCardDesc}>Фильтрация позволяет найти любой курс за
                        ...
                        <div className={coursesStyles.whyUsCard}>
                            ...
                            <div className={coursesStyles.whyUsCardDescBody}>
                                ...
                                <span className={coursesStyles.whyUsCardDesc}>Скидки на разные курсы так же система геймификации в ходе которой ученики
                        ...
                        <div className={coursesStyles.whyUsCard}>
                            ...
                            <div className={coursesStyles.whyUsCardDescBody}>
                                ...
                                <span className={coursesStyles.whyUsCardDesc}>Отзывы и оценки курсов позволят подобрать
                        ...
                        <div className={coursesStyles.whyUsCard}>
                            ...
                            <div className={coursesStyles.whyUsCardDescBody}>
                                <span className={coursesStyles.whyUsCardTitle}>Система оповещений</span>
                        ...
                        <div className={coursesStyles.whyUsCard}>
                            ...
                            <div className={coursesStyles.whyUsCardDescBody}>
                                <span className={coursesStyles.whyUsCardTitle}>Контроль качества</span>
                                <span className={coursesStyles.whyUsCardDesc}>У каждого курса формируется рейтинг исходя из отзывов студентов,
                        ...
                        <div className={coursesStyles.whyUsCard}>
                            ...
                            <div className={coursesStyles.whyUsCardDescBody}>
                                <span className={coursesStyles.whyUsCardTitle}>Новые навыки</span>
                                <span className={coursesStyles.whyUsCardDesc}>
